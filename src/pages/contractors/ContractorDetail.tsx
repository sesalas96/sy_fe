...
export const ContractorDetail: React.FC = () => {
  ...
  // Access control - allow more roles than just CLIENT_SUPERVISOR
  const allowedRoles = [
    UserRole.SUPER_ADMIN,
    UserRole.SAFETY_STAFF,
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            ...
            <Avatar
              sx={{ 
                ...
                height: { xs: 48, md: 64 }, 
                mr: { xs: 2, md: 3 }, 
                bgcolor: contractor.status === 'active' || contractor.status === 'activo' ? 'success.main' : 
                         contractor.status === 'inactive' || contractor.status === 'inactivo' ? 'warning.main' : 'error.main',
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {contractor.supervisor && (
                  <Chip 
                    label={`Sup: ${contractor.supervisor.firstName || ''} ${contractor.supervisor.lastName || ''}`.trim() || contractor.supervisor.email} 
                    size="small" 
                    variant="outlined" 
                    icon={<GroupIcon />}
                    color="info"
