          ...
          <Card>
            <CardContent>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12 }}>
                  <FormControl fullWidth>
                    <InputLabel>Frecuencia de Reportes</InputLabel>
                    <Select
                      value={supervisorSettings.reportFrequency}
                      onChange={(e) => setSupervisorSettings(prev => ({...prev, reportFrequency: e.target.value}))}
                ...
                <Grid size={{ xs: 12 }}>
                  <FormControl fullWidth>
                    <InputLabel>Tamaño de Espacios de Trabajo</InputLabel>
                    <Select
                      value={supervisorSettings.companySize}
                      onChange={(e) => setSupervisorSettings(prev => ({...prev, companySize: e.target.value}))}
                      label="Tamaño de Espacios de Trabajo"
                    >
                      <MenuItem value="small">Pequeña (1-50)</MenuItem>
                      <MenuItem value="medium">Mediana (51-250)</MenuItem>
                      <MenuItem value="large">Grande (251-1000)</MenuItem>
                      <MenuItem value="enterprise">Corporativo (1000+)</MenuItem>
                    </Select>
                  </FormControl>
                </Grid>
                ...
                <Grid size={{ xs: 12 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <FormControlLabel
                      control={
                        <Switch
                          checked={supervisorSettings.autoApproval}
                          onChange={(e) => setSupervisorSettings(prev => ({...prev, autoApproval: e.target.checked}))}
                          color="primary"
                        />
                      }
                      label="Auto-aprobación de permisos rutinarios"
                    />
                    <FormControlLabel
                      control={
                        <Switch
                          checked={supervisorSettings.teamNotifications}
                          onChange={(e) => setSupervisorSettings(prev => ({...prev, teamNotifications: e.target.checked}))}
                          color="info"
                        />
                      }
                      label="Notificaciones de equipo"
                    />
                    <FormControlLabel
                      control={
                        <Switch
                          checked={supervisorSettings.budgetAlerts}
                          onChange={(e) => setSupervisorSettings(prev => ({...prev, budgetAlerts: e.target.checked}))}
                          color="warning"
                        />
                      }
